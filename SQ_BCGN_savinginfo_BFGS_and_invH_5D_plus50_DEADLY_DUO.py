...
def SQ_BCGN_savinginfo_BFGS_and_invH_5D_plus50_DEADLY_DUO(r, J, x0, sampling_func_adaptive, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
           ...
           #      actual_previous_useful_k=k-1
             ...
             if kappa==1 and p==2: #if we are on the Gauss Southwell
                if ex_step_no==2: #if we did step in S2 do in S1
                    if k!=0:
                        Jtav_2=J(x)[:,len(S1):n]
                        J_old_GN_matrix_2=Jtav_2.T.dot(Jtav_2)
                        if np.linalg.matrix_rank(J_old_GN_matrix_2)==J_old_GN_matrix_2.shape[0]:
                            J_old_inverse_hessian_2=np.linalg.inv(J_old_GN_matrix_2)
                        else:
                            J_coefficient_n=np.linalg.norm(J_old_GN_matrix_2,ord=2)/100
                            J_old_inverse_hessian_2=np.linalg.inv(J_old_GN_matrix_2+J_coefficient_n*np.eye(J_old_GN_matrix_2.shape[0]))
                        J_T_J_grad_2=previous_grad_2+J_old_GN_matrix_2.dot(x_k[S2,k]-previous_location_2)
                        ...
                        S_hat = step_and_grad_sampling_5D(approx_step_2, approx_grad_x_2, old_step_2, previous_grad_2,J_T_J_grad_2)
                    ...
                    if k!=0:
                        Jtav_1=J(x)[:,0:len(S1)]
                        J_old_GN_matrix_1=Jtav_1.T.dot(Jtav_1)
                        if np.linalg.matrix_rank(J_old_GN_matrix_1)==J_old_GN_matrix_1.shape[0]:
                            J_old_inverse_hessian_1=np.linalg.inv(J_old_GN_matrix_1)
                        else:
                            J_coefficient_n=np.linalg.norm(J_old_GN_matrix_1,ord=2)/100
                            J_old_inverse_hessian_1=np.linalg.inv(J_old_GN_matrix_1+J_coefficient_n*np.eye(J_old_GN_matrix_1.shape[0]))
                        J_T_J_grad_1=previous_grad_1+J_old_GN_matrix_1.dot(x_k[S1,k]-previous_location_1)
                        ...
                        S_hat = step_and_grad_sampling_5D(approx_step_1, approx_grad_x_1, old_step_1, previous_grad_1,J_T_J_grad_1)
