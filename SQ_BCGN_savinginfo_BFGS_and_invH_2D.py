...
def SQ_BCGN_savinginfo_BFGS_and_invH_2D(r, J, x0, sampling_func_adaptive, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
    r0=r(x0)
    m= r0.size
           ...
           #      actual_previous_useful_k=k-1
             ...
             if kappa==1 and p==2: #if we are on the Gauss Southwell
                if block_size_k[k-1]==n: #if the previous step was full GN sample based on the "taylor" expansion of the gradient
                    ...
                    S = step_and_grad_sampling(approx_step, approx_grad_x)
...
def step_and_grad_sampling (step, grad):
    ssize=len(step)
    ...
    if np.linalg.norm(grad-grad_T_v1*v1,ord=2)!=0:
        ...
        v2=grad
        rem_inds = np.setdiff1d(np.arange(n),index)
