...
def SQ_BCGN_savinginfo_BFGS_and_invH_4D_plus50_DEADLY_DUO_BFGS_only_corrected_coupling_11_aug(r, J, x0, sampling_func_adaptive, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
    S1=np.arange(round(n/2))
    S2=np.setdiff1d(np.arange(n),S1)
    r0=r(x0)
    m= r0.size
    second_order_initialization_1=False
    second_order_initialization_2=False
    ...
    while (not fig and budget < it_max*n and k<max_number_of_gradients) or (fig and k < it_max and ma.fabs(f(x) - fxopt) > ftol):
        ...
        if steplength_k[k-1]==0:
            if (kappa!=1 or p!=2):
                ...
                if np.linalg.norm(gradf_S,ord=2)<1e-09: #correct failure due to zero grad in that subspace
                    if ex_step_no==1:
                        p_in_budget=len(S2)
                        saved_ex_step_no=1
                    else:
          ...
          #  if  actual_previous_useful_k==-1:
           #      actual_previous_useful_k=k-1
            ...
            #                old_inverse_hessian_1=np.linalg.inv(old_GN_matrix_1+coefficient_n*np.eye(old_GN_matrix_1.shape[0]))
                        ...
                        approx_grad_x_1=previous_grad_1+old_GN_matrix_1.dot(x_k[S1,k]-previous_location_1)
...
#!!!!!!!!!!!!!#end IF len(S.shape)==1!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    #save data------part 1]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]
        ...
        if k==0 and kappa==1 and p==2:#initializae the hessian -and also update it
            J1=J_upgrade
            g1=gradf_S_upgrade
            x1=x_k[:,k]
            old_GN_matrix_1=J1.T.dot(J1)
            if np.linalg.matrix_rank(old_GN_matrix_1)==old_GN_matrix_1.shape[0]:
                old_inverse_hessian_1=np.linalg.inv(old_GN_matrix_1)
            else:
                coefficient_n=np.linalg.norm(old_GN_matrix_1,ord=2)/100
        ...
        elif initialized_step_2==True and steplength_k[k]!=0 and kappa==1 and p==2:
            if ex_step_no==1: #if we took a succesful step in S1
                #if np.linalg.norm(x_k[:,k]-x2,ord=2)!=0:#upgrade the hessian in S1
                if second_order_initialization_1==False:
                    #BFGS_update_B(B11,B22,y1,s1,s2,rho):
                    current_estimated_grad_2_at_1=S_tilde.dot(gradf_S[len(S1):len(gradf_S)])
                    g1=gradf_S_upgrade
                    grad_current_1=g1
                    current_location_1=x_k[0:len(S1),k]
                    rho=1/np.dot(grad_current_1-previous_grad_1,current_location_1-previous_location_1)
                    very_old_GN_matrix_1=old_GN_matrix_1
                    very_old_inverse_hessian_1=old_inverse_hessian_1
                    old_GN_matrix_1=BFGS_update_H(old_GN_matrix_1,old_GN_matrix_2,grad_current_1-previous_grad_1,current_location_1-previous_location_1,x_k[S2,k]-x1[S2],rho)  
                    #old_inverse_hessian_1=SR1_update_B(old_inverse_hessian_1,old_inverse_hessian_2,grad_current_1-previous_grad_1,current_location_1-previous_location_1,x_k[S2,k]-x1[S2],current_estimated_grad_2_at_1)
                    old_inverse_hessian_1=BFGS_update_B(old_inverse_hessian_1,old_inverse_hessian_2,grad_current_1-previous_grad_1,current_location_1-previous_location_1,x_k[S2,k]-x1[S2],rho,current_estimated_grad_2_at_1)
                    
                    previous_grad_1=grad_current_1
                    previous_location_1=current_location_1
                    second_order_initialization_1=True
                    previous_estimated_grad_2_at_1=S_tilde.dot(gradf_S[len(S1):len(gradf_S)])
                    previous_estimated_full_grad_1=np.hstack((g1,previous_estimated_grad_2_at_1))
                    x1=x_k[:,k]
                else:
                    current_estimated_grad_2_at_1=S_tilde.dot(gradf_S[len(S1):len(gradf_S)])
                    g1=gradf_S_upgrade
                    grad_current_1=g1
                    estimated_full_grad_1=np.hstack((g1,current_estimated_grad_2_at_1))
                    estimated_full_grad_change=estimated_full_grad_1-previous_estimated_full_grad_1
                    current_location_1=x_k[0:len(S1),k]
                    rho=1/np.dot(estimated_full_grad_change,x_k[:,k]-x1)
                    very_old_GN_matrix_1=old_GN_matrix_1
                    very_old_inverse_hessian_1=old_inverse_hessian_1
                    old_GN_matrix_1=BFGS_update_H(old_GN_matrix_1,0.5*(old_GN_matrix_2+very_old_GN_matrix_2),grad_current_1-previous_grad_1,current_location_1-previous_location_1,x_k[S2,k]-x1[S2],rho)  
                 #   old_inverse_hessian_1=SR1_update_B(old_inverse_hessian_1,0.5*(old_inverse_hessian_2+very_old_inverse_hessian_2),grad_current_1-previous_grad_1,current_location_1-previous_location_1,x_k[S2,k]-x1[S2],current_estimated_grad_2_at_1)
                    old_inverse_hessian_1=BFGS_update_B(old_inverse_hessian_1,0.5*(old_inverse_hessian_2+very_old_inverse_hessian_2),grad_current_1-previous_grad_1,current_location_1-previous_location_1,x_k[S2,k]-x1[S2],rho,current_estimated_grad_2_at_1)
                    
                    previous_grad_1=grad_current_1
                    previous_location_1=current_location_1
                    second_order_initialization_1=True
                 #   previous_estimated_grad_2_at_1=current_estimated_grad_2_at_1
                    previous_estimated_full_grad_1=estimated_full_grad_1
                    x1=x_k[:,k]
                
                
            elif ex_step_no==2 and np.linalg.norm(x_k[len(S1):n,k]-previous_location_2)!=0: #else if we did a succ step in S2
               if second_order_initialization_2==False:
                    #BFGS_update_B(B11,B22,y1,s1,s2,rho):
                    current_estimated_grad_1_at_2=S_tilde.dot(gradf_S[len(S2):len(gradf_S)])
                    g2=gradf_S_upgrade
                    grad_current_2=g2
                    current_location_2=x_k[S2,k]
                    rho=1/np.dot(grad_current_2-previous_grad_2,current_location_2-previous_location_2)
                    very_old_GN_matrix_2=old_GN_matrix_2
                    very_old_inverse_hessian_2=old_inverse_hessian_2
                    old_GN_matrix_2=BFGS_update_H(old_GN_matrix_2,old_GN_matrix_1,grad_current_2-previous_grad_2,current_location_2-previous_location_2,x_k[S1,k]-x2[S1],rho)  
                    #old_inverse_hessian_2=SR1_update_B(old_inverse_hessian_2,old_inverse_hessian_1,grad_current_2-previous_grad_2,current_location_2-previous_location_2,x_k[S1,k]-x2[S1],current_estimated_grad_1_at_2)
                    old_inverse_hessian_2=BFGS_update_B(old_inverse_hessian_2,old_inverse_hessian_1,grad_current_2-previous_grad_2,current_location_2-previous_location_2,x_k[S1,k]-x2[S1],rho,current_estimated_grad_1_at_2)
                    
                    previous_grad_2=grad_current_2
                    previous_location_2=current_location_2
                    second_order_initialization_2=True
                    previous_estimated_grad_1_at_2=S_tilde.dot(gradf_S[len(S2):len(gradf_S)])
                    previous_estimated_full_grad_2=np.hstack((previous_estimated_grad_1_at_2,g2))
                    x2=x_k[:,k]
               else:
                    current_estimated_grad_1_at_2=S_tilde.dot(gradf_S[len(S2):len(gradf_S)])
                    g2=gradf_S_upgrade
                    grad_current_2=g2
                    estimated_full_grad_2=np.hstack((current_estimated_grad_1_at_2,g2))
                    estimated_full_grad_change=estimated_full_grad_2-previous_estimated_full_grad_2
                    current_location_2=x_k[S2,k]
                    rho=1/np.dot(estimated_full_grad_change,x_k[:,k]-x2)
                    very_old_GN_matrix_2=old_GN_matrix_2
                    very_old_inverse_hessian_2=old_inverse_hessian_2
                    old_GN_matrix_2=BFGS_update_H(old_GN_matrix_2,0.5*(old_GN_matrix_1+very_old_GN_matrix_1),grad_current_2-previous_grad_2,current_location_2-previous_location_2,x_k[S1,k]-x2[S1],rho)  
               #     old_inverse_hessian_2=SR1_update_B(old_inverse_hessian_2,0.5*(old_inverse_hessian_1+very_old_inverse_hessian_1),grad_current_2-previous_grad_2,current_location_2-previous_location_2,x_k[S1,k]-x2[S1],current_estimated_grad_1_at_2)
                    old_inverse_hessian_2=BFGS_update_B(old_inverse_hessian_2,0.5*(old_inverse_hessian_1+very_old_inverse_hessian_1),grad_current_2-previous_grad_2,current_location_2-previous_location_2,x_k[S1,k]-x2[S1],rho,current_estimated_grad_1_at_2)
                    
                    previous_grad_2=grad_current_2
                    previous_location_2=current_location_2
                    second_order_initialization_2=True
                 #   previous_estimated_grad_2_at_1=current_estimated_grad_2_at_1
                    previous_estimated_full_grad_2=estimated_full_grad_2
                    x2=x_k[:,k]
...
def DFP_update_H(B11,B22,y1,s1,s2,rho):
    #H is the HESSIAN not the inferse and this is rather the DFP update than the BFGS
 #   rho=1/delta_grad.dot(delta_x)
    if np.isnan(rho):
        return B11
    bracket1=(np.eye(B11.shape[0])-rho*np.outer(y1,s1))
    bracket2=(np.eye(B11.shape[0])-rho*np.outer(s1,y1))
    NEW_TERM=B22.dot(np.outer(s2,y1))
    CC=np.outer(y1,s2)
    NEW_TERM=CC.dot(NEW_TERM)
    A=np.dot(bracket1,np.dot(B11,bracket2))+rho*np.outer(y1,y1)+rho*rho*NEW_TERM
    return A
def BFGS_update_H(H11,H22,y1,s1,s2,rho):
    if np.isnan(rho):
        return H11
    S11=np.outer(s1,s1)
    S22=np.outer(s2,s2)
    term1=H11.dot(S11.dot(H11.T))
    s_T_H_s=s1.dot(H11.dot(s1))+s2.dot(H22.dot(s2))
    A=H11-1/s_T_H_s*term1+np.outer(y1,y1)*rho
    return A
def SR1_update_B(B11,B22,y1,s1,s2,y2):
    #B22,s2 and rho are dummies, NOT USED just not to change function call brackets
    v1=s1-B11.dot(y1)
    v2=s2-B22.dot(y2)
    denom=v1.dot(y1)+v2.dot(y2)
    if denom<1e-06:
        return B11
    else:
        return B11+np.outer(v1,v1)*1/denom
def BFGS_update_B(B11,B22,y1,s1,s2,rho,y2):
    #H is the HESSIAN not the inferse and this is rather the DFP update than the BFGS
 #   rho=1/delta_grad.dot(delta_x)
    if np.isnan(rho):
        return B11
    bracket1=(np.eye(B11.shape[0])-rho*np.outer(s1,y1))
    bracket2=(np.eye(B11.shape[0])-rho*np.outer(y1,s1))
    NEW_TERM=B22.dot(np.outer(y2,s1))
    CC=np.outer(s1,y2)
    NEW_TERM=CC.dot(NEW_TERM)
    A=np.dot(bracket1,np.dot(B11,bracket2))+rho*np.outer(s1,s1)+rho*rho*NEW_TERM
    return A
#def BFGS_update_hessian(H,delta_grad,delta_x):
    #H is the HESSIAN not the inferse and this is rather the DFP update than the BFGS
 #   rho=1/delta_grad.dot(delta_x)
  #  if np.isnan(rho):
   #     return H
    #C1=H.dot(delta_x)
    #denom=np.dot(delta_x,C1)
    #return H-np.outer(C1,C1)/denom+rho*np.outer(delta_grad,delta_grad)
