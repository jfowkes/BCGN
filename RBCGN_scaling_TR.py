...
def RBCGN_scaling_TR(r, J, x0, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
    ...
    while (not fig and budget < it_max*n) or (fig and k < it_max and ma.fabs(f(x) - fxopt) > ftol):
       ...
       #=======================================================================   '''      
        ...
        #---------------------------------------------------------------------
        if np.linalg.norm(x-x_k[:,k],ord=2)==0 and p==n:
            budget=budget-n
        #save data------part 1-------------   
        JJ=J(x)
        grad[:,k]=JJ.T.dot(rx)
