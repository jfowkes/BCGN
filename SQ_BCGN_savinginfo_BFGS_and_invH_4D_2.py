...
import pickle
import os
#contains BFGS update of hessian as well rather than DFP for hessian and BFGS for invhessian
def SQ_BCGN_savinginfo_BFGS_and_invH_4D_2(r, J, x0, sampling_func_adaptive, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
    r0=r(x0)
    m= r0.size
    ...
    while (not fig and budget < it_max*n and k<max_number_of_gradients) or (fig and k < it_max and ma.fabs(f(x) - fxopt) > ftol):
        ...
        if steplength_k[k-1]==0:
            if (kappa!=1 or p!=2):
                ...
                if len(S.shape)!=1:
                    ...
                    if np.linalg.norm(gradf_S,ord=2)<1e-08:
                        S=np.arange(n)
                        p_in_budget=n-4
                    else:
                        S=S
           ...
           #      actual_previous_useful_k=k-1
             ...
             if kappa==1 and p==2: #if we are on the Gauss Southwell
                if block_size_k[k-1]==n: #if the previous step was full GN sample based on the "taylor" expansion of the gradient
                    ...
                    #Directions_matrix=np.vstack((approx_step, approx_grad_x, old_step, old_grad)).T
                    #S = modifiedGramSchmidt(Directions_matrix)
        ...
        # Set initial trust region radius-----------------------------------
            if k == 0 and algorithm.startswith('tr') or algorithm == 'reg':
                ...
                if delta == 0:
                    delta = 1
            if k!=0:
                delta=max(Delta_exploration,delta) #i.e. only allow exploitation to increase delta but not decrease it
                #however until the step is taken the exploitation stage functions normally
        #--------------------------------------------------------------------
        ...
        # Iteratively refine block size=======Adaptive Block Size=============
            ...
            #2 Solve subproblem------------------------------------------------
                if algorithm == 'tr':
                   # L_S = np.linalg.cholesky(J_S.T.dot(J_S))
                    s_S = trs(J_S, gradf_S, delta)
                 #   s_S = trs(J_S, gradf_S, delta)
                elif algorithm == 'tr_approx':
                    s_S = trs_approx(J_S, J_ST, gradf_S, delta)
...
#!!!!!!!!!!!!!#end IF len(S.shape)==1!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    #save data------part 1]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]
        ...
        if k==0:#initializa the hessian
            ...
            #grad_current=grad[:,k]
            old_GN_matrix=BFGS_update_hessian(old_GN_matrix,x_k[:,k]-previous_location,grad_current-previous_full_grad)
...
def modifiedGramSchmidt(A):
    ...
    """
    #STILL NEED TO DO THE RANDOM VECTOR PRODUCTION TRICK WHEN LINEARLY DEPENDENT VECTORS 
    #ARE USED AS FOR THE CLASSICAL GRAMSCHMIDT - AS WE DID BEFORE
