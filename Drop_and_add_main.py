        ...
        for ikappa, kappa in enumerate(kappas):
            ...
            for ip, p in enumerate(blocks):
                ...
                for iseed, seed in enumerate(seeds):
                    ...
                    # Run RBCGN
                    if PLOT: # Plotting
                        Ys[:,:,iseed] = RBCGN_drop_and_add(r,J,x0,sampling_func_drop,sampling_func,fxopt,IT_MAX,FTOL,p,fig,kappa,func,iseed,algorithm=ALG)
                    else: # performance profiles - THIS IS WHERE RBCGN is called
                        measures[ifunc,ishift+ip,:,iseed] = RBCGN_drop_and_add(r,J,x0,sampling_func_drop,sampling_func,fxopt,IT_MAX,FTOL,p,None,kappa,func,iseed,algorithm=ALG)
