...
def SQ_BCGN_savinginfo_BFGS_and_invH_5D_plus50_DEADLY_DUO_Improved(r, J, x0, sampling_func_adaptive, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
          ...
          #  if  actual_previous_useful_k==-1:
           #      actual_previous_useful_k=k-1
             ...
             #               old_inverse_hessian_2=np.linalg.inv(old_GN_matrix_2+coefficient_n*np.eye(old_GN_matrix_2.shape[0]))
                        ...
                        J_1_current=J(x)[:,S1]
                        gradf_S1_current=J_1_current.T.dot(r(x))
                        current_full_grad_estimate=np.hstack((gradf_S1_current,approx_grad_x_2))
                        delta_grad_full=current_full_grad_estimate-old_full_grad_estimate
                        old_full_grad_estimate=current_full_grad_estimate
                        s_full=x-x2
                        Full_inverse_hessian=BFGS_update(Full_inverse_hessian,s_full,delta_grad_full)  
                        approx_GN_Step_full=Full_inverse_hessian.dot(current_full_grad_estimate)
                        approx_step_2=approx_GN_Step_full[S2]
                        
                        old_step_2=x_k[S2,k]-previous_location_2
                        approx_step_2_usual=old_inverse_hessian_2.dot(approx_grad_x_2)
                        S_hat = step_and_grad_sampling_5D(approx_step_2, approx_grad_x_2, old_step_2, previous_grad_2,approx_step_2_usual)
            ...
            #                old_inverse_hessian_1=np.linalg.inv(old_GN_matrix_1+coefficient_n*np.eye(old_GN_matrix_1.shape[0]))
                        ...
                        J_2_current=J(x)[:,S2]
                        gradf_S2_current=J_2_current.T.dot(r(x))
                        current_full_grad_estimate=np.hstack((gradf_S2_current,approx_grad_x_1))
                        delta_grad_full=current_full_grad_estimate-old_full_grad_estimate
                        old_full_grad_estimate=current_full_grad_estimate
                        s_full=x-x1
                        Full_inverse_hessian=BFGS_update(Full_inverse_hessian,s_full,delta_grad_full)  
                        approx_GN_Step_full=Full_inverse_hessian.dot(current_full_grad_estimate)
                        approx_step_1=approx_GN_Step_full[S1]
                        
                        approx_step_1_usual=old_inverse_hessian_1.dot(approx_grad_x_1)
                        old_step_1=x_k[S1,k]-previous_location_1
                        S_hat = step_and_grad_sampling_5D(approx_step_1, approx_grad_x_1, old_step_1, previous_grad_1,approx_step_1_usual)
...
#!!!!!!!!!!!!!#end IF len(S.shape)==1!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    #save data------part 1]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]
        ...
        if k==0 and kappa==1 and p==2:#initializae the hessian -and also update it
            Jfull=J(x0)
            Full_inverse_hessian=Jfull.T.dot(Jfull) #due to 1st step full GN
            x2=x_k[:,k] #due to 1st step full GN
            old_full_grad_estimate=J(x0).T.dot(r(x0))
            ...
            if ex_step_no==1: #if we took a succesful step in S1
                ...
                old_GN_matrix_1=BFGS_update(old_GN_matrix_1,grad_current_1-previous_grad_1,current_location_1-previous_location_1)
                old_inverse_hessian_1=BFGS_update(old_inverse_hessian_1,current_location_1-previous_location_1,grad_current_1-previous_grad_1)  
               ...
               old_GN_matrix_2=BFGS_update(old_GN_matrix_2,grad_current_2-previous_grad_2,current_location_2-previous_location_2)
               old_inverse_hessian_2=BFGS_update(old_inverse_hessian_2,current_location_2-previous_location_2,grad_current_2-previous_grad_2)  
...
def step_and_grad_sampling_4D (step, grad, old_step, old_grad):
    ...
    S=S.T
    return S
def step_and_grad_sampling_5D (step, grad, old_step, old_grad, J_T_J_grad):
    ssize=len(step)
    if np.linalg.norm(step,ord=2)!=0:
        v1=1/np.linalg.norm(step,ord=2)*step
        index=np.random.choice(np.arange(ssize),size=1,replace=False)
    ...
    J_T_J_grad_v1=J_T_J_grad.dot(v1)
    J_T_J_grad_v2=J_T_J_grad.dot(v2)
    J_T_J_grad_v3=J_T_J_grad.dot(v3)
    J_T_J_grad_v4=J_T_J_grad.dot(v4)
    
    if np.linalg.norm(J_T_J_grad-J_T_J_grad_v1*v1-J_T_J_grad_v2*v2-J_T_J_grad_v3*v3-J_T_J_grad_v4*v4,ord=2)!=0:
        v5=J_T_J_grad-J_T_J_grad_v1*v1-J_T_J_grad_v2*v2-J_T_J_grad_v3*v3-J_T_J_grad_v4*v4
        v4=1/np.linalg.norm(v5,ord=2)*v5
    else:
        v5=old_grad+J_T_J_grad
        rem_inds = np.setdiff1d(np.arange(ssize),index)
        indices=np.random.choice(rem_inds,size=int(np.round(ssize/1.5)),replace=False)
        v5[indices]=v5[indices]-0.1*np.linalg.norm(old_grad+0.1,ord=2)
        indices=np.random.choice(rem_inds,size=int(np.round(ssize/1.5)),replace=False)
        v5[indices]=v5[indices]-0.1*np.linalg.norm(old_grad+0.1,ord=2)
        v5=1/np.linalg.norm(v5,ord=2)*v5 
    S=np.array([v1,v2,v3,v4,v5])
    S=S.T
    return S
