...
import pickle
import os
'THIS WAS NOT RUN FOR TR_UPDATE_ONLY_ON_EXPLORATION_STEPS BUT THE REQUIRED CHANGES WERE MADE'

def SQ_BCGN_savinginfo_BFGS_and_invH(r, J, x0, sampling_func_adaptive, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
    r0=r(x0)
    m= r0.size
    ...
    while (not fig and budget < it_max*n and k<max_number_of_gradients) or (fig and k < it_max and ma.fabs(f(x) - fxopt) > ftol):
        ...
        if steplength_k[k-1]==0:
            if (kappa!=1 or p!=2):
                ...
                if len(S.shape)!=1:
                    S,V,measure_vect,p_in_budget = principal_component_sampling_increase(S,V,measure_vect)
                else:
                    S=np.arange(n)
                    p_in_budget=0
           ...
           #      actual_previous_useful_k=k-1
             ...
             if kappa==1 and p==2: #if we are on the Gauss Southwell
                if block_size_k[k-1]==n: #if the previous step was full GN sample based on the "taylor" expansion of the gradient
                    ...
                    S,V,measure_vect = principal_component_sampling(old_inverse_hessian, approx_grad_x)
