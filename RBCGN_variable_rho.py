...
def RBCGN_variable_rho(r, J, x0, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
    ...
    while (not fig and budget < it_max*n) or (fig and k < it_max and ma.fabs(f(x) - fxopt) > ftol):
        ...
        Js_S = J_S.dot(s_S)
        Delta_m = (-np.dot(gradf_S,s_S) -0.5*np.dot(Js_S,Js_S))*n/len(S)
