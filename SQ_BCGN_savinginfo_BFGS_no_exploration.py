...
def SQ_BCGN_savinginfo_BFGS_no_exploration(r, J, x0, sampling_func_adaptive, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
        ...
        if steplength_k[k-1]==0:
          ...
          #  if  actual_previous_useful_k==-1:
           #      actual_previous_useful_k=k-1
            ...
            #    if  actual_previous_useful_k!=-1:
             #       kk= actual_previous_useful_k
             #   else:
              #      kk=k-1
                if k==0: #on the first step do GN
                    S=np.arange(n)
                else:  
                    old_grad=grad[:,k-1]
                    #Jac=J_saved_k[k-1]
                    approx_grad_x=old_grad+old_GN_matrix.dot(x_k[:,k]-x_k[:,k-1])
                    S = greedy_gauss_southwell(n,p,approx_grad_x)
                p_in_budget = len(S)
             else:
 ...
 #save data------part 1]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]
        ...
        elif np.linalg.norm(x_k[:,k]-previous_location,ord=2)!=0:# and np.linalg.norm(grad_current-previous_full_grad)!=0: #and len(S)==n: #subsequently, do a BFG update only on fullGN steps
            #for only on ful steps uncomment and...
            grad_current=np.zeros(n)
            grad_current[S]=grad[S,k]
            #grad_current=grad[:,k]
            old_GN_matrix=BFGS_update_c(old_GN_matrix,grad_current-previous_full_grad,x_k[:,k]-previous_location)
...
def BFGS_update(H,delta_grad,delta_x):
    ...
    if np.isnan(rho):#if either grad or delta x is zero
        return H
