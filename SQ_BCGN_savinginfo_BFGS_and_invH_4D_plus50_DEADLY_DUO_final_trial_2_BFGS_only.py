...
def SQ_BCGN_savinginfo_BFGS_and_invH_4D_plus50_DEADLY_DUO_final_trial_2_BFGS_only(r, J, x0, sampling_func_adaptive, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
          ...
          #  if  actual_previous_useful_k==-1:
           #      actual_previous_useful_k=k-1
             ...
             #               old_inverse_hessian_2=np.linalg.inv(old_GN_matrix_2+coefficient_n*np.eye(old_GN_matrix_2.shape[0]))
                        ...
                        approx_grad_x=previous_grad+old_GN_matrix.dot(x_k[:,k]-previous_location)
                        new_current_grad=np.hstack((J(x).T.dot(r(x))[S1],approx_grad_x[S2]))
                        new_inverse_hessian=BFGS_update(old_inverse_hessian,x_k[:,k]-previous_location,new_current_grad-previous_grad)  
                        approx_step=new_inverse_hessian.dot(approx_grad_x)
                        
                        approx_grad_x_2=approx_grad_x[S2]
                        approx_step_2=approx_step[S2]
            ...
            #                old_inverse_hessian_1=np.linalg.inv(old_GN_matrix_1+coefficient_n*np.eye(old_GN_matrix_1.shape[0]))
                        ...
                        approx_grad_x=previous_grad+old_GN_matrix.dot(x_k[:,k]-previous_location)
                        new_current_grad=np.hstack((approx_grad_x[S1],J(x).T.dot(r(x))[S2]))
                        new_inverse_hessian=BFGS_update(old_inverse_hessian,x_k[:,k]-previous_location,new_current_grad-previous_grad)  
                        approx_step=new_inverse_hessian.dot(approx_grad_x)
                        
                        approx_grad_x_1=approx_grad_x[S1]
                        approx_step_1=approx_step[S1]
...
#!!!!!!!!!!!!!#end IF len(S.shape)==1!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    #save data------part 1]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]
        ...
        if k==0 and kappa==1 and p==2:#initializae the hessian -and also update it
            ...
            previous_grad=J_S.T.dot(r(previous_location))
            previous_grad_1=previous_grad[S1]
            previous_grad_2=previous_grad[S2]
            previous_location_1=x_k[0:len(S1),k]
            previous_location_2=x_k[len(S1):n,k]
            initialized=True
        elif initialized==True and steplength_k[k]!=0 and kappa==1 and p==2:
            if ex_step_no==1: #if we took a succesful step in S1
                #if np.linalg.norm(x_k[:,k]-x2,ord=2)!=0:#upgrade the hessian in S1
                grad_current_1=gradf_S_upgrade
                x1=x_k[:,k];                current_location=x1
                estimated_gradf_S2=S_tilde.dot(gradf_S[len(S1):len(gradf_S)])
                grad_current=np.hstack((gradf_S_upgrade,estimated_gradf_S2))
                
                old_GN_matrix=BFGS_update_hessian(old_GN_matrix,current_location-previous_location,grad_current-previous_grad)  
                old_inverse_hessian=BFGS_update(old_inverse_hessian,current_location-previous_location,grad_current-previous_grad)  
                previous_grad=grad_current
                previous_location=current_location
                previous_grad_1=previous_grad[S1]
                previous_grad_2=previous_grad[S2]
                previous_location_1=previous_location[S1]
                previous_location_2=previous_location[S2]
            elif ex_step_no==2 and np.linalg.norm(x_k[len(S1):n,k]-previous_location_2)!=0: #else if we did a succ step in S2
               grad_current_2=gradf_S_upgrade #upgrade the hessian in S2 np.linalg.norm(x_k[:,k]-x1,ord=2)!=0 and 
               x2=x_k[:,k];               current_location=x2
               estimated_gradf_S1=S_tilde.dot(gradf_S[len(S2):len(gradf_S)])
               grad_current=np.hstack((estimated_gradf_S1,gradf_S_upgrade))
               ...
               previous_location_1=previous_location[S1]
               previous_location_2=previous_location[S2]
            elif ex_step_no==0 and np.linalg.norm(x_k[:,k]-previous_location)!=0: #else if we did full GN step and TAKEN the step
               x2=x_k[:,k]
               x1=x_k[:,k]
               grad_current=gradf_S
               current_location=x2
