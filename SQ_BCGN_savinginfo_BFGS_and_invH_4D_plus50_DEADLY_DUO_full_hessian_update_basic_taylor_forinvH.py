...
def SQ_BCGN_savinginfo_BFGS_and_invH_4D_plus50_DEADLY_DUO_full_hessian_update_basic_taylor_forinvH(r, J, x0, sampling_func_adaptive, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
    ...
    ex_step_no=2
    delta_initialized_step_2=False
          ...
          #  if  actual_previous_useful_k==-1:
           #      actual_previous_useful_k=k-1
             ...
             #               old_inverse_hessian_2=np.linalg.inv(old_GN_matrix_2+coefficient_n*np.eye(old_GN_matrix_2.shape[0]))
                      ...
                      #  old_inverse_hessian_2=old_inverse_hessian[len(S1):n,len(S1):n]
                       ...
                       # approx_step_2=old_inverse_hessian_2.dot(approx_grad_x_2)
                        ...
                        J_S1 = J(x)[:,S1]
                        rx = r(x)
                        computed_gradient_1= J_S1.T.dot(rx)
                        
                        approx_step_2=old_inverse_hessian_2.dot(np.hstack((computed_gradient_1,previous_grad_2)))+(x_k[S2,k]-previous_location_full[S2])
            ...
            #                old_inverse_hessian_1=np.linalg.inv(old_GN_matrix_1+coefficient_n*np.eye(old_GN_matrix_1.shape[0]))
                      ...
                      #  old_inverse_hessian_1=old_inverse_hessian[0:len(S1),0:len(S1)]
                        old_inverse_hessian_1=old_inverse_hessian[0:len(S1),:]
                        
                        
                        J_S2 = J(x)[:,S2]
                        rx = r(x)
                        computed_gradient_2= J_S2.T.dot(rx)
                        
                        approx_grad_x_1=previous_grad_1+old_GN_matrix_1.dot(x_k[:,k]-previous_location_full)
                        approx_step_1=old_inverse_hessian_1.dot(np.hstack((previous_grad_1,computed_gradient_2)))+(x_k[S1,k]-previous_location_full[S1])
...
#!!!!!!!!!!!!!#end IF len(S.shape)==1!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       #     if ex_step_no==1 :
        #        delta=0.001
         #   elif delta_initialized_step_2==True: #THE DELTA TRICK
          #      delta=old_delta
           # elif delta_initialized_step_2==False:
            #    delta_initialized_step_2=True
             #   delta = linalg.norm(gradf_S)/5
              #  if delta == 0:
               #     delta = 1
