...
def SQ_BCGN_savinginfo_BFGS_and_invH_4D_50_COMPROMISE(r, J, x0, sampling_func_adaptive, sampling_func, fxopt, it_max, ftol, p, fig, kappa, function_name, run_counter,algorithm='tr'):
    n = x0.size
    r0=r(x0)
    m= r0.size
          ...
          #  if  actual_previous_useful_k==-1:
           #      actual_previous_useful_k=k-1
             ...
             if kappa==1 and p==2: #if we are on the Gauss Southwell
                if block_size_k[k-1]==round(n/2) and ex_step_no==2: #exploitation STEP
                    old_grad=grad[:,k-1]
                    #Jac=J_saved_k[k-1]
                    approx_grad_x=old_grad+old_GN_matrix.dot(x_k[:,k]-previous_location)
                    approx_step=old_inverse_hessian.dot(approx_grad_x)
                    old_step=x_k[:,k]-previous_location
                    old_grad=previous_full_grad
                    S = step_and_grad_sampling_4D(approx_step, approx_grad_x, old_step, old_grad)
                    p_in_budget=S.shape[1]
                    ex_step_no=0
                    x_0_0=x
                elif block_size_k[k-1]==round(n/2) and ex_step_no==1: #exploitation step 2
                    S=np.arange(n)[round(n/2):n]
                    p_in_budget=len(S)
                    ex_step_no=2
                else:   #exploitation stn=ep 1 #note this works with N EVEN ONLY!!!!
                    S=np.arange(round(n/2))
                    p_in_budget = len(S)
                    ex_step_no=1
             else:
...
#!!!!!!!!!!!!!#end IF len(S.shape)==1!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    #save data------part 1]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]
        ...
        if k==0:#initializae the hessian -and also update it
            J1=J_S
            g1=gradf_S
            x1=x
        elif k>0 and steplength_k[k]!=0 and initialized_step_2==False: #initialization part 2
            g2=gradf_S
            J2=J_S
            x2=x
            ...
            previous_full_grad=np.hstack((g1,g2))
            previous_location=0.5*(x1+x2)
            initialized_step_2=True
        elif initialized_step_2==True and steplength_k[k]!=0 and kappa==1 and p==2:
            if ex_step_no==1:
                if np.linalg.norm(x_k[:,k]-x_0_0,ord=2)!=0:
                    g1=gradf_S
                    x1=x
            elif np.linalg.norm(x_k[:,k]-x1,ord=2)!=0 and ex_step_no==2: #subsequently, do a BFG update only on fullGN steps
                g2=gradf_S
                x2=x
                grad_current=np.hstack((g1,g2))
                current_location=0.5*(x2+x1)
                #grad_current=grad[:,k]
                old_GN_matrix=BFGS_update(old_GN_matrix,grad_current-previous_full_grad,current_location-previous_location)
                old_inverse_hessian=BFGS_update(old_inverse_hessian,current_location-previous_location,grad_current-previous_full_grad)
