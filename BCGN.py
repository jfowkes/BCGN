...
def main():
    ...
    NO_INSTANCES = 40# No. random runs
    FTOL = 1e-10
    ALG = 'tr'
    #-------------------------------------------------------------------------
    # Plotting parameters-----------------------------------------------------
        ...
        for ikappa, kappa in enumerate(kappas):
            ...
            for ip, p in enumerate(blocks):
                ...
                for iseed, seed in enumerate(seeds):
                    ...
                    # Run RBCGN
                    if PLOT: # Plotting
                        Ys[:,:,iseed] = RBCGN(r,J,x0,sampling_func,fxopt,IT_MAX,FTOL,p,fig,kappa,func,algorithm=ALG)
                    else: # performance profiles - THIS IS WHERE RBCGN is called
                        measures[ifunc,ishift+ip,:,iseed] = RBCGN(r,J,x0,sampling_func,fxopt,IT_MAX,FTOL,p,None,kappa,func,algorithm=ALG)
