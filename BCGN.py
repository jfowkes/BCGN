        ...
        for ikappa, kappa in enumerate(kappas):
            ...
            for ip, p in enumerate(blocks):
                ...
                for iseed, seed in enumerate(seeds):
                    ...
                    # Run RBCGN
                    if PLOT: # Plotting
                        Ys[:,:,iseed] = GRBCGN(r,J,x0,sampling_func,fxopt,IT_MAX,FTOL,p,fig,kappa,func,iseed,algorithm=ALG)
                    else: # performance profiles - THIS IS WHERE RBCGN is called
                        measures[ifunc,ishift+ip,:,iseed] = GRBCGN(r,J,x0,sampling_func,fxopt,IT_MAX,FTOL,p,None,kappa,algorithm=ALG)
                        #RBCGN returns nan if not solved or no of coord eval if solved
                                                       #     GRBCGN(r, J, x0, sampling_func, fxopt, it_max, ftol, p, fig, kappa, algorithm='tr'):
